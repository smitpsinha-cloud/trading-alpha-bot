...
def cancel_settlement(sid, admin_id):
    with db() as con:
        s=con.execute("SELECT * FROM settlements WHERE id=%s FOR UPDATE",(sid,)).fetchone()
        if not s or s["status"]!="PENDING":
            return None
        con.execute(
            "UPDATE settlements SET status='CANCELLED',confirmed_by=%s,completed_at=NOW() WHERE id=%s",
            (admin_id,sid)
        )
        con.execute(
            "UPDATE users SET status='ACTIVE',opt_out_at=NULL WHERE id=%s",
            (s["user_id"],)
        )
        return con.execute(
            "SELECT u.telegram_id,s.final_amount FROM settlements s JOIN users u ON u.id=s.user_id WHERE s.id=%s",
            (sid,)
        ).fetchone()
