...
async def confirm_settle(update,context):
    ...
    await q.edit_message_text(
        f"⏳ OPT-OUT REQUEST SUBMITTED\\n\\nSettlement #{sid}\\n"
        f"Final amount: {money(s['final_amount'])}\\n\\n"
        "The Admin must now approve the settlement before payment can be recorded."
    )

    admin_kb=InlineKeyboardMarkup([
        [InlineKeyboardButton("💸 Proceed to Payment",callback_data=f"settlepay:{sid}")],
        [InlineKeyboardButton("↩️ Cancel Opt-Out",callback_data=f"settlecancel:{sid}")]
    ])
    for aid in ADMIN_IDS:
        try:
            await context.bot.send_message(
                aid,
                f"🔴 OPT-OUT REQUEST #{sid}\\n\\n"
                f"User: @{s['username'] or 'none'}\\n"
                f"Telegram ID: {s['telegram_id']}\\n"
                f"Original amount: {money(s['original_amount'])}\\n"
                f"Today's payout status: {s['todays_payout_status']}\\n"
                f"Today's payout: {money(s['todays_payout'])}\\n"
                f"FINAL SETTLEMENT: {money(s['final_amount'])}\\n\\n"
                "Choose how to proceed:",
                reply_markup=admin_kb
            )
        except Exception as e:
            print("Admin opt-out notify:",e)

async def admin_cancel_settlement(update,context):
    q=update.callback_query
    if q.from_user.id not in ADMIN_IDS:
        await q.answer("Not authorized.",show_alert=True); return
    await q.answer()
    sid=int(q.data.split(":")[1])
    r=database.cancel_settlement(sid,q.from_user.id)
    if not r:
        await q.edit_message_text("⚠️ Settlement was already processed or could not be found.")
        return
    await q.edit_message_text(f"↩️ OPT-OUT CANCELLED\\n\\nSettlement #{sid} was cancelled. User remains ACTIVE.")
    await context.bot.send_message(
        r["telegram_id"],
        "↩️ OPT-OUT REQUEST CANCELLED BY ADMIN\\n\\n"
        "Your opt-out settlement was not processed and your account remains active. "
        "Your daily payout eligibility continues."
    )
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(admin_cancel_settlement,pattern="^settlecancel:"))
