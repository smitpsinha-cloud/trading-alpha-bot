...
def payout(amount): return amount * RATE // 100
...
async def account(update, context):
    ...
    if not u:
        await q.edit_message_text("Please send /start first."); return
    await q.edit_message_text(
        f"💰 MY ACCOUNT\n\nStatus: {u['status']}\nActive amount: {money(u['active_amount'])}\n"
        f"Daily payout: {money(payout(u['active_amount']))}\n\n"
        "Your active amount changes only after a payment is verified.",
        reply_markup=main_menu()
    )
...
async def add_amount_start(update, context):
    ...
    if not u or u["status"] not in ("ACTIVE","PENDING"):
        await q.edit_message_text("Please send /start and opt in first."); return ConversationHandler.END
    remaining=MAX_ACTIVE-u["active_amount"]
    if remaining < MIN_ADD:
        await q.edit_message_text("Your active amount has reached the current ₹10,000 maximum."); return ConversationHandler.END
    await q.edit_message_text(
        f"➕ ADD / INCREASE AMOUNT\n\nCurrent active amount: {money(u['active_amount'])}\n"
        f"Maximum total: {money(MAX_ACTIVE)}\n\n"
        f"Enter the additional amount (minimum {money(MIN_ADD)}).\n"
        "Example: 2500\n\n/cancel to cancel."
    )
    return AMOUNT
...
async def amount_received(update, context):
    ...
    await update.message.reply_text(
        f"Payment amount: {money(amount)}\n\nUPI ID to pay:\n{PROGRAM_UPI_ID}\n\n"
        "After making the payment, enter the UPI ID you paid from."
    )
    return USER_UPI
...
async def user_upi_received(update, context):
    ...
    if len(v)<3:
        await update.message.reply_text("Please enter a valid UPI ID."); return USER_UPI
...
async def utr_received(update, context):
    v=update.message.text.strip()
    if len(v)<4:
        await update.message.reply_text("Please enter a valid UTR / transaction ID."); return UTR
...
async def screenshot_received(update, context):
    ...
    amount=context.user_data["amount"]; upi=context.user_data["upi"]; utr=context.user_data["utr"]
    fid=update.message.photo[-1].file_id
    did=database.create_deposit(tid,amount,upi,utr,fid)
    await update.message.reply_text(
        f"✅ PAYMENT SUBMITTED\n\nAmount: {money(amount)}\nUTR: {utr}\n\n"
        "Your payment is pending Admin verification. Your active amount will not change until verification."
    )
    kb=InlineKeyboardMarkup([[
        InlineKeyboardButton("✅ Verify",callback_data=f"verify:{did}"),
...
REASONS = {
    "utr":"UTR not found / could not be verified",
    "amount":"Amount mismatch",
    "duplicate":"Duplicate transaction",
    "screenshot":"Screenshot unclear",
    "other":"Other"
}
...
async def admin_action(update, context):
    ...
    if action=="verify":
        ...
        if r=="LIMIT":
            await q.edit_message_caption(caption=(q.message.caption or "")+"\n\n❌ Cannot verify: ₹10,000 limit would be exceeded.")
            return
        if not r:
            await q.edit_message_caption(caption=(q.message.caption or "")+"\n\n⚠️ Already processed or not found."); return
        await q.edit_message_caption(caption=(q.message.caption or "")+"\n\n✅ VERIFIED")
        await context.bot.send_message(r["telegram_id"],
            f"✅ PAYMENT VERIFIED\n\nVerified amount: {money(r['amount'])}\n"
            f"Active amount: {money(r['active_amount'])}\nDaily payout: {money(payout(r['active_amount']))}")
    else:
        kb=InlineKeyboardMarkup([
            [InlineKeyboardButton("UTR not found",callback_data=f"reason:{did}:utr"),
             InlineKeyboardButton("Amount mismatch",callback_data=f"reason:{did}:amount")],
            [InlineKeyboardButton("Duplicate",callback_data=f"reason:{did}:duplicate"),
             InlineKeyboardButton("Screenshot unclear",callback_data=f"reason:{did}:screenshot")],
...
async def reject_reason(update, context):
    ...
    await context.bot.send_message(d["telegram_id"],
        f"❌ PAYMENT REJECTED\n\nAmount: {money(d['amount'])}\nReason: {REASONS[reason]}\n\n"
        "Your active amount has not been increased. You may submit the payment again after correcting the issue.")

async def payment_status(update, context):
    q=update.callback_query; await q.answer()
    rows=database.pending_deposits()
    mine=[r for r in rows if r["telegram_id"]==q.from_user.id]
    if mine:
        text="💳 PENDING PAYMENT\n\n"+"\n".join(f"Deposit #{r['id']}: {money(r['amount'])} — Pending" for r in mine)
    else:
        text="💳 PAYMENT STATUS\n\nNo pending payment was found for your account."
    await q.edit_message_text(text,reply_markup=main_menu())

async def opt_out(update, context):
    q=update.callback_query; await q.answer()
    u=database.get_user(q.from_user.id)
    if not u or u["active_amount"]<=0:
        await q.edit_message_text("There is no active amount to settle.",reply_markup=main_menu()); return
    p=database.get_today_payout_for_user(q.from_user.id,today())
    if p and p["status"]=="PAID":
        status="PAID"; today_amt=p["payout_amount"]
    else:
        status="UNPAID"; today_amt=p["payout_amount"] if p else payout(u["active_amount"])
    ...
    explanation=(f"Today's payout: {money(today_amt)} — already paid.\n"
                 f"Final settlement: {money(final)}" if status=="PAID" else
                 f"Today's payout: {money(today_amt)} — not yet paid.\nFinal settlement: {money(final)}")
...
async def confirm_settle(update, context):
    q=update.callback_query
    if q.from_user.id != q.from_user.id: pass
    await q.answer()
    s=database.get_settlement(int(q.data.split(":")[1]))
    if not s:
    ...
    for aid in ADMIN_IDS:
        await context.bot.send_message(aid,
            f"🔴 OPT-OUT SETTLEMENT #{s['id']}\nUser: @{s['username'] or 'none'}\n"
            f"Original: {money(s['original_amount'])}\nToday's payout status: {s['today_payout_status']}\n"
            f"Today's payout: {money(s["today's_payout"])}\nFINAL: {money(s['final_amount'])}")

async def cancel_optout(update, context):
    q=update.callback_query; await q.answer()
    await q.edit_message_text("Opt-out cancelled.",reply_markup=main_menu())

async def admin_payouts(update, context):
    if update.effective_user.id not in ADMIN_IDS: return
    rows=database.pending_payouts(today())
    if not rows:
        await update.message.reply_text("No pending payouts for today."); return
...
async def payout_start(update, context):
    ...
    await q.answer()
    pid=int(q.data.split(":")[1]); context.user_data["payout_id"]=pid
    await q.message.reply_text("Enter payout UTR / transaction ID.")
    return PAYOUT_UTR

async def payout_utr(update, context):
    context.user_data["payout_utr"]=update.message.text.strip()
    await update.message.reply_text("Upload the payout screenshot.")
    return PAYOUT_SCREENSHOT

async def payout_screenshot(update, context):
    if not update.message.photo:
        await update.message.reply_text("Upload the screenshot as a photo."); return PAYOUT_SCREENSHOT
    ...
    if not r:
        await update.message.reply_text("Payout already processed or not found."); return ConversationHandler.END
    await update.message.reply_text("✅ Payout recorded.")
    await context.bot.send_photo(r["telegram_id"],update.message.photo[-1].file_id,
        caption=f"✅ DAILY PAYOUT COMPLETED\nDate: {r['payout_date']}\nAmount: {money(r['payout_amount'])}\nUTR: {context.user_data['payout_utr']}")
    context.user_data.clear()
    return ConversationHandler.END
...
async def admin_settlements(update, context):
    ...
    if not rows:
        await update.message.reply_text("No pending settlements."); return
    for r in rows:
...
async def settlement_start(update, context):
    q=update.callback_query
    if q.from_user.id not in ADMIN_IDS: await q.answer("Not authorized.",show_alert=True); return ConversationHandler.END
    await q.answer(); context.user_data["settlement_id"]=int(q.data.split(":")[1])
    await q.message.reply_text("Enter settlement UTR.")
    return SETTLE_UTR

async def settlement_utr(update, context):
    context.user_data["settlement_utr"]=update.message.text.strip()
    await update.message.reply_text("Upload settlement screenshot.")
    return SETTLE_SCREENSHOT

async def settlement_screenshot(update, context):
    if not update.message.photo:
        await update.message.reply_text("Upload the screenshot as a photo."); return SETTLE_SCREENSHOT
    ...
    if not r:
        await update.message.reply_text("Settlement already processed or not found."); return ConversationHandler.END
    await update.message.reply_text("✅ Settlement recorded.")
    await context.bot.send_photo(r["telegram_id"],update.message.photo[-1].file_id,
        caption=f"✅ FINAL SETTLEMENT PAID\nAmount: {money(r['final_amount'])}\nYour account is now closed.")
    context.user_data.clear()
    return ConversationHandler.END

async def payout_job(context):
    date=today()
    rows=database.create_today_payouts(date,RATE)
    total=sum(x[1] for x in rows)
    for aid in ADMIN_IDS:
        try: await context.bot.send_message(aid,f"⏰ DAILY PAYOUT QUEUE — 10:00 AM\nDate: {date}\nUsers: {len(rows)}\nTotal due: {money(total)}\nDeadline: 1:00 PM\n\nUse /payouts")
        except: pass
...
def main():
    ...
    if not ADMIN_IDS: raise RuntimeError("Set ADMIN_IDS environment variable.")
    ...
    app.add_handler(settlement_conv)
    ...
    # Daily 10:00 IST. JobQueue runs using the server clock; calculate next run in IST.
    import datetime as dt
    now=datetime.now(IST)
    first=now.replace(hour=10,minute=0,second=0,microsecond=0)
    if first <= now: first += dt.timedelta(days=1)
    delay=(first-now).total_seconds()
    app.job_queue.run_repeating(payout_job, interval=86400, first=delay)

    print("Bot is running. Daily payout job scheduled for 10:00 IST.")
    app.run_polling()
